bot.modules.unload = function(from, to, text, message) {
  if (from === bot.config.master) {
    ...
    try {
      ...
      bot.speak('\u000303Unloaded module' + ((text.split(' ').length>1)?'s':'') + ' \u000f[\u000310' + text.replace(/ /g, ', ') + '\u000f]')
    } catch(err) {
      console.log('Error unloading module [' + text + ']')
      bot.speak('\u000304Error unloading module \u000f[\u000310' + text + '\u000f]')
    }
  }
}
...
bot.modules.load = function(from, to, text, message) {
  if (from === bot.config.master) {
    ...
    try {
      ...
      bot.speak('\u000303Loaded module' + ((text.split(' ').length>1)?'s':'') + ' \u000f[\u000310' + text.replace(/ /g, ', ') + '\u000f]')
    } catch(err) {
      console.log('Error loading module [' + text + ']')
      bot.speak('\u000304Error loading module \u000f[\u000310' + text + '\u000f]')
    }
  }
}
